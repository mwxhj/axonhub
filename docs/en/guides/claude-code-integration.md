...
AxonHub no longer maintains a separate Claude Code provider-quota polling subsystem.

If you need budgeting or routing control for Claude Code credentials, configure local credential quota on the credential itself. Upstream quota failures are treated as ordinary request failures and handled through normal fallback behavior.
