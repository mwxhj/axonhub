...
Do not reintroduce channel-local or credential-local budget quota as a routing product concept. If consumer/org/project budgeting is added later, design it outside this channel/credential provider quota contract.
...
Local quota APIs are not part of the product-facing route availability model:
...
Do not add new UI or runtime availability dependencies on these local quota APIs.
...
- Runtime route availability is derived from `channel.status`, `ref.enabled`, `credential.status`, model/route eligibility, and credential/key provider quota readiness.
- Runtime route availability must not consult channel-local quota or `CredentialQuotaScope` local budget state.
...
- `CredentialQuotaScope` local quota is not a visible product concept for this model. Existing fields or GraphQL inputs are legacy until a cleanup migration removes them.
...
| Local quota scope is exhausted/paused/disabled | Do not use it to block route availability under this model. |
...
- Bad: using `CredentialQuotaScope.status` to block route availability.
