...
func TestNewUniversalOptions(t *testing.T) {
	...
	t.Run("redis addrs override addr", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, []string{"127.0.0.1:26379", "127.0.0.1:26380"}, opts.Addrs)
	})
	...
	t.Run("redis cluster mode with config", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, []string{"127.0.0.1:7001", "127.0.0.1:7002"}, opts.Addrs)
...
func TestParseUniversalURL(t *testing.T) {
	...
	t.Run("short URL has no implicit host", func(t *testing.T) {
		opts, err := ParseUniversalURL("redis://")
		assert.NoError(t, err)
		assert.Empty(t, opts.Addrs)
	})

	t.Run("full URL", func(t *testing.T) {
		opts, err := ParseUniversalURL("rediss://user:pass@127.0.0.1:6379/1?" + strings.Join([]string{
			"client_name=axonhub_client",
			"db=2",
			"protocol=2",
			"username=user1",
			"password=pass1",
	...
	t.Run("invalid query bool is surfaced", func(t *testing.T) {
		_, err := ParseUniversalURL("redis://127.0.0.1:6379?route_by_latency=maybe")
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "invalid route_by_latency boolean")
	})
}
