...
func TestRequestService_UpdateRequestCompletedWithAudio_ExternalStorage(t *testing.T) {
	...
	systemService := NewSystemService(SystemServiceParams{CacheConfig: xcache.Config{Mode: xcache.ModeMemory}, Ent: client})
	...
	reqRow, err := client.Request.Create().
		...
		SetRequestBody(objects.JSONRawMessage(`{"model":"tts-1","input":"hi","voice":"alloy"}`)).
	...
	placeholder := objects.JSONRawMessage(`{"object":"audio.speech","content_type":"audio/mpeg","bytes":7}`)
	...
	storedAudio, err := dataStorageService.LoadData(ctx, ds, *updated.ContentStorageKey)
	require.NoError(t, err)
	require.Equal(t, audio, storedAudio)
}
