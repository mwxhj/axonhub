...
// IsNode implements the Node interface check for GQLGen.
func (*CredentialQuotaScope) IsNode() {}
...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case channelprobe.Table:
		...
		return query.Only(ctx)
	case credentialquotascope.Table:
		query := c.CredentialQuotaScope.Query().
			Where(credentialquotascope.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, credentialquotascopeImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case credentialquotascope.Table:
		query := c.CredentialQuotaScope.Query().
			Where(credentialquotascope.IDIn(ids...))
		query, err := query.CollectFields(ctx, credentialquotascopeImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
